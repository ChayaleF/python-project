...
async def test_sign_up_user():
    ...
    mock_user = User(id=1, name="testuser", password="Qqssdd2@g", email="test@j.com")
    ...
    result = await login("testuser", "Qqssdd2@g")
    print(result)

    # Verify that the user exists in the database
    assert len(list(collection.find({"name": "testuser", "password": "Qqssdd2@g"}))) == 1

    # Clean up the database by deleting the mock user
    collection.delete_one({"name": "testuser", "password": "Qqssdd2@g"})
